...
public class AuctionService extends AbstractService implements ClientSessionChannel.MessageListener, BayeuxServer.ChannelListener, BayeuxServer.SubscriptionListener {
    ...
    public AuctionService(ServletContext context) {
        ...
        getBayeux().createChannelIfAbsent("/service" + AUCTION_ROOT + "*", channel -> channel.addAuthorizer(GrantAuthorizer.GRANT_ALL));
        getBayeux().createChannelIfAbsent(AUCTION_ROOT + "*", channel -> channel.addAuthorizer(GrantAuthorizer.GRANT_ALL));
