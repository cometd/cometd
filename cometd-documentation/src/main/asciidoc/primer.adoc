...
Each browser has its own version of the Developer Tools, where you typically have
a JavaScript console tool available, a network trace tool, a CSS editor tool, etc.

The CometD project is built using https://maven.apache.org[Maven], and using Maven
...
You can set up the project in two ways: using xref:_primer_maven_way[the Maven way]
or xref:_primer_non_maven_way[the non Maven way].
For both, you can follow xref:_primer_setup_details[setup section]
...
----
$ cd /tmp
$ mvn org.apache.maven.plugins:maven-archetype-plugin:2.4:generate -DarchetypeCatalog=https://cometd.org
...
Choose archetype:
...
xref:_java_server[the Java server library section].
...
* The use of the function `_metaConnect()` to detect when the communication has
  ...
  be https://en.wikipedia.org/wiki/Idempotent[idempotent].
