...
xref:_javascript_handshake[the handshake section]) before you can publish.
...
`/meta/publish` channel is not part of xref:_bayeux[the Bayeux protocol].
...
to use xref:_javascript_batch[the javascript batch section].
...
xref:_javascript_rpc_binary[remote calls with binary data].
...
xref:_javascript_configure[parameter]. A negative value or `0` disables the timeout.
...
xref:_java_server_services_annotated[annotated service], in particular one with a
`@RemoteCall` xref:_java_server_services_annotated_server_side_remote_call[annotation].
...
Similarly to xref:_javascript_publish_binary[publishing binary data], it is possible
