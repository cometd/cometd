...
similar mechanism).
See the <<_concepts_application_authentication,asynchronous authentication examples>>
below.
...
The <<_bayeux,Bayeux protocol>> message format is the JSON format, because the
...
<<_extensions_binary,the binary extension section>>.
...
<<_java_server_services_annotated_server_side_binary,the binary services section>>.
