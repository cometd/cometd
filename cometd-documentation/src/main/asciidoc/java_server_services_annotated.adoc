...
By annotating fields with the standard `@Inject` annotation, or the standard `@PostConstruct` and `@PreDestroy` annotations you obtain the same functionality.
...
The BayeuxServer field is annotated with the Jakarta `@Inject` annotation.
The Jakarta `@Inject` annotation is supported also by other dependency injection frameworks (such as Spring).

===== Lifecycle Management Support

The CometD project provides lifecycle management via the Jakarta `@PostConstruct` and `@PreDestroy` annotations.
...
The CometD project does not offer dependency injection for client-side services, but supports lifecycle management via the Jakarta `@PostConstruct` and `@PreDestroy` annotations.
