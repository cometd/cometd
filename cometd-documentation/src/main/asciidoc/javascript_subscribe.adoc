...
var subscription1 = cometd.addListener("/meta/connect", (m) => { ... });
var subscription2 = cometd.subscribe("/foo/bar/", (m) => { ... });
...
----
// Some initialization code.
var subscription1 = cometd.subscribe("/foo/bar/", () => { ... });
...
This information is already known to CometD at the moment `cometd.subscribe(...)` was called (above in method `dynamicSubscribe()`), so applications can just call `resubscribe()` using the subscription object obtained from `subscribe()`:
