...
xref:_java_server_authorization[the authorization section]), `Authorizer` methods
are invoked for any `ServerSession`, even those generated by local clients
(such as services, see also xref:_java_server_services_inherited[the inherited services section]).
