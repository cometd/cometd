...
----
====
<1> Make `MyAppAuthenticator` be a `SecurityPolicy` and a `ServerSession.RemovedListener`,
...
<7> Implement `RemovedListener.removed()`, which is called when the remote session
...
[source,java]
----
public class MyAppAuthenticator extends DefaultSecurityPolicy implements ServerSession.RemovedListener {
...
removed, therefore there is no need to register a `RemovedListener` with the
