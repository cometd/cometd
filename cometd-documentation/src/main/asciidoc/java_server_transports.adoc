...
If you have configured your web application to support cross-origin HTTP calls (see also xref:_java_server_configuration_advanced[this section]), you do not need to worry because the `CrossOriginFilter` is by default disabled for the WebSocket protocol.
