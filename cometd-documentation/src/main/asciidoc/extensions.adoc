...
xref:_concepts_message_processing[message processing] (an _incoming_ extension),
...
the xref:_bayeux_ext[`ext` object] that the Bayeux protocol specification defines.
...
with https://en.wikipedia.org/wiki/Network_Time_Protocol[NTP]), etc.
...
xref:_javascript_configure[the JavaScript library configuration section].
====
