...
<4> Subscribes to channel `/echo`, and specifies the name of a method that must be called when a message arrives to that channel, via `addService(...)`.
...
Notice that the channel name specified to the `addService(...)` method may be a wildcard, for example:
