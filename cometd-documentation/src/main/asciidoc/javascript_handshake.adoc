...
xref:_javascript_configure[the javascript configuration section]) followed by
a call to `handshake()`.
...
`/meta/handshake` listener (see also xref:_javascript_subscribe_vs_listen[this section]).
...
xref:_javascript_publish[the javascript publish section]) can actually succeed in contacting
the Bayeux server.
...
(see also xref:_java_server_services_annotated[the annotated services section]):
...
reserved fields defined by the Bayeux protocol (see also xref:_bayeux[the Bayeux protocol section]).
...
xref:_javascript_subscribe[the javascript subscribe section], or by passing callback
