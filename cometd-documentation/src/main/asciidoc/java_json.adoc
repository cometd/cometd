...
It is enough that the client formats the JSON object adding an additional `class` field whose value is the fully qualified class name that you want to convert the JSON to:
...
On the server, in the `web.xml` file, you register the `org.cometd.server.JettyJSONContextServer` as `jsonContext` (see also xref:_java_json_server_config[the JSON server configuration section]), and at startup you add a custom converter for the `org.cometd.example.EchoInfo` class (see also xref:_java_server_services_integration[the services integration section] for more details about configuring CometD at startup).
