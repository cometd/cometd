...
xref:_javascript_handshake[the JavaScript handshake section] or the
xref:_java_client_handshake[the Java client handshake section].

==== Binary Data

CometD now allows to send/receive messages with binary data, see
xref:_concepts_binary_data[the binary data section].
...
The xref:_extensions_reload[reload extension] has been rewritten to use the
