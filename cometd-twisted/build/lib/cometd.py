...
class cometd(resource.PostableResource):
	...
	def render(self, request):
		...
		while len(messages):
			...
			m = messages.pop(0)
			if verbose:
				log.msg("tmp message:", type(m), ":",  json_encode(m))
			...
			if SupportedFixedTopics.has_key(m["channel"]):
				...
				if isinstance(value, dict):
					...
					if m.has_key("clientId"):
						if CometClients.connectedClients.has_key(m["clientId"]):
							...
							value["error"] = 'true'
							log.msg("Throwing away message %s for client %s, client has disconnected." % (pprint.pformat(value),m["clientId"]))
							return buildResponse(request, 'Message from invalid or disconnected client, ignored', 500, "text/plain")
					resp.append(value)
				else:
					resp = value
					break
				...
				if CometClients.connectedClients.has_key(m["clientId"]):
					...
					log.msg("Throwing away message %s from client %s for other clients, originating client has disconnected." % (pprint.pformat(resp),m["clientId"]))
					return buildResponse(request, 'Message from invalid client, ignored', 500, "text/plain")
	...
	def _sanityCheckConnection(self, request, message):
		...
		if not message.has_key("clientId") or not CometClients.connectedClients.has_key(message["clientId"]):
			isSane = False
	...
	def route(self, request, message):
		...
		'''
		Note that group name is only registered/validated during subscribe().
		It is stored in the data structure, but not re-validated with each message.
		(Should probably be moved out of the data structure and just treated
		a member of the topic class).
		'''
		already_sent = Set([])
		...
		if ClientTopics.quickTopicMatch.activeTopics.has_key(message_topic):
			...
			if no_match != []:
				...
				if verbose: log.msg("Program Error: these client_ids are in the quick Topic match, but are not connected: %s, the message was not routed." % pprint.pformat(no_match))
				
			return result

		result["successful"] = False
		result["error"] = [1301,BayeuxServer.json_errcodes[1301] % (message_topic)]
		if verbose: log.msg("No topic:", message_topic, "matches for delivery. If implicit subscribe is allowed, it will be done now.")
		return result
