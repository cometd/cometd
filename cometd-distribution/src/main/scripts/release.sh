...
if yes_no "Relink documentation ? (Y/n)" y; then
  ...
  ssh ubuntu@docs.cometd.org "sudo -u www-data bash -c 'cd ${DOCS_ROOT} && ln -fns ${VERSION} current6'"
fi

echo "Updating cometd-javascript repository"
COMETD_JS_DIR=${COMETD_DIR}/target/release/cometd-javascript
git clone --branch 6.0.x git@github.com:cometd/cometd-javascript.git ${COMETD_JS_DIR}
